...
import { Sms, MultiSms } from "../model/SmsModel";
...
export class SmsService {
    ...
    public async sendMultiSms(data) {
        const sms = new MultiSms(data.phoneNumbers, data.content);
        const errors = await validate(sms);
        if (errors.length > 0) {
            throw Error(errors.toString());
        }

        const json = { to: sms.phoneNumbers, content: sms.content, sms_type: 2, brandname: "" };
        request({
            headers: {
                Authorization: Constant.SMS_TOKEN
            },
